...
def run(x_test,mnist,y_test): 
    ...
    with tf.Session() as sess:
        ...
        for i in range(1000):
            ...
            temp1 = batch_y1.astype(np.int32)
            temp1[temp1 !=1] = -1
            
            batch_x2, batch_y2 = mnist2.train.next_batch(batch_size)
            temp2 = batch_y2.astype(np.int32)
            temp2[temp2 !=2] = -1          
            
            batch_x3, batch_y3 = mnist3.train.next_batch(batch_size)
            temp3 = batch_y3.astype(np.int32)
            temp3[temp3 !=3] = -1   
    
            batch_x4, batch_y4 = mnist4.train.next_batch(batch_size)
            temp4 = batch_y4.astype(np.int32)
            temp4[temp4 !=4] = -1
            
            batch_x5, batch_y5 = mnist5.train.next_batch(batch_size)
            temp5 = batch_y5.astype(np.int32)
            temp5[temp5 !=5] = -1
            
            batch_x6, batch_y6 = mnist6.train.next_batch(batch_size)
            temp6 = batch_y6.astype(np.int32)
            temp6[temp6 !=6] = -1
            
            batch_x7, batch_y7 = mnist7.train.next_batch(batch_size)
            temp7 = batch_y7.astype(np.int32)
            temp7[temp7 !=7] = -1
            
            batch_x8, batch_y8 = mnist8.train.next_batch(batch_size)
            temp8 = batch_y8.astype(np.int32)
            temp8[temp8 !=8] = -1
            
            batch_x9, batch_y9 = mnist9.train.next_batch(batch_size)
            temp9 = batch_y9.astype(np.int32)
            temp9[temp9 !=9] = -1
            
            batch_x10, batch_y10 = mnist10.train.next_batch(batch_size)
            temp10 = batch_y10.astype(np.int32)
            temp10[temp10 !=0] = -1
            
            #batch_y = batch_y.reshape(batch_size,1)
            
            training1.run( feed_dict={x1:batch_x1, y1:temp1})
            training2.run( feed_dict={x2:batch_x2, y2:temp2})
            training3.run( feed_dict={x3:batch_x3, y3:temp3})
            training4.run( feed_dict={x4:batch_x4, y4:temp4})
            training5.run( feed_dict={x5:batch_x5, y5:temp5})
            training6.run( feed_dict={x6:batch_x6, y6:temp6})
            training7.run( feed_dict={x7:batch_x7, y7:temp7})
            training8.run( feed_dict={x8:batch_x8, y8:temp8})
            training9.run( feed_dict={x9:batch_x9, y9:temp9})
            training10.run( feed_dict={x10:batch_x10, y10:temp10})

...
def net(input, is_training, dropout_kept_prob):
  ...
  conv2 = tf.nn.max_pool(conv2, ksize = [1, 2, 2, 1], strides = [1, 2, 2, 1], padding = 'VALID') #(5, 5, 16)
  print(conv2)
  ...
  print(fl1)
  fl1_w = tf.Variable(tf.truncated_normal(shape = (400, 120), mean = mu, stddev = sigma), name = 'fl1_w')
  fl1_b = tf.Variable(tf.zeros(120), name = 'fl1_b')
  fl2 = tf.matmul(fl1, fl1_w) + fl1_b
  #activation function relu
  fl2 = tf.nn.relu(fl2) #(120)
  print("fl2", fl2)
  
  '''forth fully connected layer'''
  fl2_w = tf.Variable(tf.truncated_normal(shape = (120, 84), mean = mu, stddev = sigma), name = 'fl2_w') 
